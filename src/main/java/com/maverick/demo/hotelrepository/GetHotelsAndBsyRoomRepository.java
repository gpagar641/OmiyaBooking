...
public interface GetHotelsAndBsyRoomRepository extends JpaRepository<GetHotelsAndBsyRoom, Integer>{
	...
	@Query(value="select h.*, pic.image_name,coalesce((select sum(bd.no_of_rooms) from t_booking_details bd where bd.hotel_id=h.hotel_id and "
			...
			+ "as ac_bsy_room,c.city_name from m_hotels h,m_city c, m_images pic where h.hotel_id=pic.hotel_id AND h.city_id=:cityId and "
			+ "h.del_status=1 and c.city_id=h.city_id group by pic.hotel_id",nativeQuery=true)
	...
	@Query(value="select h.*, pic.image_name,coalesce((select sum(bd.no_of_rooms) from t_booking_details bd where bd.hotel_id=h.hotel_id and "
			...
			+ "from t_booking_details bd where   bd.hotel_id=h.hotel_id and bd.status=0 and bd.room_type=1 and "
			+ "((bd.booking_from between :fromDate and :toDate) or (bd.booking_to between :fromDate and :toDate)) ),0) "
			+ "as ac_bsy_room,c.city_name from m_hotels h,m_city c, m_images pic where h.hotel_id=pic.hotel_id  and "
			+ "h.del_status=1 and c.city_id=h.city_id group by pic.hotel_id",nativeQuery=true)
	List<GetHotelsAndBsyRoom> getHotelListWithBsyRoom(@Param("fromDate") String fromDate,@Param("toDate") String toDate);
